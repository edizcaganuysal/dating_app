...
function MemberRow({ member, index, interested, blocked, onToggleInterest, onToggleBlock }: {
  member: GroupMember; index: number; interested: boolean; blocked: boolean;
  onToggleInterest: () => void; onToggleBlock: () => void;
}) {
    ...
    <Animated.View style={fadeStyle}>
      <View style={styles.memberRow} testID={`interest-${member.user_id}`}>
        ...
        <View style={styles.memberActions}>
          <TouchableOpacity testID={`heart-${member.user_id}`}
            style={[styles.heartButton, interested && styles.heartActive]} onPress={onToggleInterest}>
            <Text style={[styles.heartIcon, interested && styles.heartIconActive]}>
              {interested ? '\u2764' : '\u2661'}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity testID={`block-${member.user_id}`}
            style={[styles.blockButton, blocked && styles.blockActive]} onPress={onToggleBlock}>
...
export default function PostDateScreen() {
  ...
  const otherMembers: GroupMember[] = group ? group.members.filter((m) => m.user_id !== user?.id) : [];

  const toggleBlock = (userId: string) => {
    Alert.alert('Block this person?', "You won't be matched with them again.", [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Block', style: 'destructive', onPress: () => {
        setBlockIds((prev) => prev.includes(userId) ? prev.filter((id) => id !== userId) : [...prev, userId]);
      }},
    ]);
  };

  const handleSubmit = async () => {
    if (rating === 0) { Alert.alert('Please rate your experience', 'Tap the stars to rate 1-5.'); return; }
    setSubmitting(true);
    try {
      const romanticInterests: RomanticInterestInput[] = otherMembers.map((m) => ({
        user_id: m.user_id, interested: interests[m.user_id] || false,
      }));
      await submitFeedback(groupId, {
        experience_rating: rating, romantic_interests: romanticInterests,
        block_user_ids: blockIds, report_user_ids: reportUserId ? [reportUserId] : [],
    ...
    } catch { Alert.alert('Error', 'Failed to submit feedback. Please try again.'); }
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Text style={styles.subtitle}>{group.activity.replace(/_/g, ' ')} on {group.scheduled_date}</Text>

      <Text style={styles.sectionTitle}>Experience Rating</Text>
      <View style={styles.starsRow} testID="star-rating">
        {[1, 2, 3, 4, 5].map((star) => (
          <Pressable key={star} onPress={() => { haptic.selection(); setRating(star); }} hitSlop={8} style={styles.starTouchTarget} testID={`star-${star}`}>
            <Ionicons name={rating >= star ? 'star' : 'star-outline'} size={36} color={rating >= star ? '#FFD700' : colors.grayLight} />
          </Pressable>
        ))}
      </View>
      ...
      {otherMembers.map((member, index) => (
        <MemberRow key={member.user_id} member={member} index={index}
          interested={interests[member.user_id] || false} blocked={blockIds.includes(member.user_id)}
          onToggleInterest={() => setInterests((prev) => ({ ...prev, [member.user_id]: !prev[member.user_id] }))}
          onToggleBlock={() => toggleBlock(member.user_id)} />
      ))}

      <TouchableOpacity testID="report-button" style={styles.reportButton} onPress={() => setReportModalVisible(true)}>
        <Text style={styles.reportButtonText}>Report a member</Text>
      </TouchableOpacity>

      <PressableScale testID="submit-button" style={[styles.submitButton, submitting && styles.submitButtonDisabled]} onPress={handleSubmit} disabled={submitting}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            {otherMembers.map((member) => (
              <TouchableOpacity key={member.user_id} style={[styles.modalOption, reportUserId === member.user_id && styles.modalOptionActive]}
            ...
            {REPORT_CATEGORIES.map((cat) => (
              <TouchableOpacity key={cat.key} style={[styles.modalOption, reportCategory === cat.key && styles.modalOptionActive]}
                onPress={() => setReportCategory(cat.key)}>
                <Text>{cat.label}</Text>
              </TouchableOpacity>
            ))}
            <Text style={styles.modalLabel}>Description (optional)</Text>
            <TextInput style={styles.modalInput} value={reportDescription} onChangeText={setReportDescription} placeholder="Tell us more..." multiline />
            <View style={styles.modalActions}>
              <TouchableOpacity style={styles.modalCancel} onPress={() => setReportModalVisible(false)}><Text>Cancel</Text></TouchableOpacity>
              <TouchableOpacity style={styles.modalSubmit} onPress={() => setReportModalVisible(false)}><Text style={styles.modalSubmitText}>Done</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  memberRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: colors.border },
  memberInfo: { flexDirection: 'row', alignItems: 'center' },
  memberName: { fontSize: 16, fontWeight: '500' },
  memberActions: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  heartButton: { width: 40, height: 40, borderRadius: 20, borderWidth: 2, borderColor: colors.primary, justifyContent: 'center', alignItems: 'center' },
  ...
  blockButton: { paddingHorizontal: 10, paddingVertical: 6, borderRadius: 4, borderWidth: 1, borderColor: colors.border },
  blockActive: { backgroundColor: '#ffebee', borderColor: '#f44336' },
  blockText: { fontSize: 12, color: colors.darkSecondary },
  reportButton: { marginTop: 24, padding: 12, alignItems: 'center', borderWidth: 1, borderColor: colors.border, borderRadius: 8 },
  reportButtonText: { color: colors.darkSecondary, fontSize: 14 },
  submitButton: { marginTop: 20, marginBottom: 40, backgroundColor: colors.primary, paddingVertical: 16, borderRadius: 25, alignItems: 'center' },
  submitButtonDisabled: { backgroundColor: colors.grayLight },
  submitButtonText: { color: '#fff', fontSize: 16, fontWeight: '600' },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' },
  modalContent: { backgroundColor: colors.surfaceElevated, borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 20, maxHeight: '80%' },
